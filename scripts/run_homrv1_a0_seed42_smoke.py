"""Run 1-seed smoke for AGML-BR + A0 + Aff-ACR + HOMR-v1.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Seed42 smoke for head-only materialization auxiliary (HOMR-v1)"
    )
    parser.add_argument(
        "--config",
        default="configs/asqp_rest15_agmlbr_a0_affacr_homrv1.yaml",
        help="HOMR-v1 YAML config",
    )
    parser.add_argument(
        "--output_root",
        default=None,
        help=(
            "Output root (default: "
            "outputs/stage2_e2e_agmlbr_a0_homrv1_seed42_smoke_<timestamp>)"
        ),
    )
...
def main() -> None:
    ...
    if args.output_root:
        ...
        output_root = (
            PROJECT_ROOT / f"outputs/stage2_e2e_agmlbr_a0_homrv1_seed42_smoke_{ts}"
        ).resolve()
    ...
    train_cfg = cfg_dir / "homrv1_a0_train.yaml"
    testeval_cfg = cfg_dir / "homrv1_a0_testeval.yaml"
    ...
    seed = int(args.seed)
    run_name = f"A0_seed{seed}_agmlbr_affacr_homrv1"
    ...
    if args.force or not stage1_ckpt.exists():
        cmd = [
            ...
            "--purpose", "smoke",
            "--run_tag", "agmlbr_affacr_homrv1_smoke",
        ]
    ...
    print("Next: run scripts/summarize_homrv1_a0_seed42_smoke.py")
