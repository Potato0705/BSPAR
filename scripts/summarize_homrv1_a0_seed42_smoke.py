...
baseline AGML-BR+A0 vs strongest Aff-ACR only vs HOMR-v1 variant.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize HOMR-v1 seed42 smoke")
    parser.add_argument(
        "--homr_output_root",
        required=True,
        help="Output root of run_homrv1_a0_seed42_smoke.py",
    )
    parser.add_argument(
        "--baseline_summary",
...
def collect_stage1(stage1_ckpt: str) -> dict:
    ...
    return {
        ...
        "homr_active_aspects": to_float(metrics.get("homr_active_aspects")),
        "homr_active_pairs": to_float(metrics.get("homr_active_pairs")),
        "homr_mean_pos_score": to_float(metrics.get("homr_mean_pos_score")),
        "homr_mean_hardneg_score": to_float(metrics.get("homr_mean_hardneg_score")),
        "homr_violation_rate": to_float(metrics.get("homr_violation_rate")),
        "homr_loss_mean": to_float(metrics.get("homr_loss_mean")),
...
def row_from_homr(homr_root: Path, seed: int) -> dict:
    manifests = list(homr_root.glob("runs/*/run_manifest.json"))
    if not manifests:
        raise RuntimeError(f"No run_manifest.json under: {homr_root / 'runs'}")
    m = load_json(manifests[0])
    if int(m["seed"]) != seed:
        raise RuntimeError(f"HOMR run seed mismatch: expected {seed}, got {m['seed']}")
...
def main() -> None:
    args = parse_args()
    seed = int(args.seed)
    homr_root = Path(args.homr_output_root).resolve()
    summary_dir = homr_root / "summary"
    ...
    homr_row = row_from_homr(homr_root, seed)

    rows = [baseline_row, affacr_row, homr_row]
    fields = [
        "track",
        "seed",
        "dev_quad_f1",
        ...
        "homr_active_pairs",
        "homr_mean_pos_score",
        "homr_mean_hardneg_score",
        "homr_violation_rate",
        "homr_loss_mean",
        "run_dir",
        "stage1_ckpt",
    ]
    compare_csv = summary_dir / "homrv1_smoke_compare_seed42.csv"
    write_csv(compare_csv, rows, fields)

    delta_rows = []
    for ref in [baseline_row, affacr_row]:
        delta = {"from_track": ref["track"], "to_track": homr_row["track"]}
        for k in fields:
    ...
    delta_csv = summary_dir / "homrv1_smoke_key_deltas_seed42.csv"
    write_csv(delta_csv, delta_rows, delta_fields)

    takeaways = [
        "# HOMR-v1 Smoke (Seed42)",
        "",
        f"- baseline test: {baseline_row['test_quad_f1']:.6f}",
        f"- strongest affacr test: {affacr_row['test_quad_f1']:.6f}",
        f"- homr-v1 test: {homr_row['test_quad_f1']:.6f}",
        f"- delta vs baseline: {homr_row['test_quad_f1'] - baseline_row['test_quad_f1']:+.6f}",
        f"- delta vs strongest affacr: {homr_row['test_quad_f1'] - affacr_row['test_quad_f1']:+.6f}",
    ]
    md = summary_dir / "homrv1_smoke_takeaways_seed42.md"
    md.write_text("\n".join(takeaways) + "\n", encoding="utf-8")
