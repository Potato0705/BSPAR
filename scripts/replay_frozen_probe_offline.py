"""Frozen-feature offline probe replay on fixed Aff-ACR checkpoint.
...
- Replay baseline vs probe-calibrated ordering strictly inside the same retained set
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Frozen-feature retained-only probe replay (no trunk retraining)"
    )
    p.add_argument(
        "--affacr_per_seed",
        default=(
    ...
    p.add_argument(
        ...
        type=float,
        default=0.10,
        help="calibrated_score = pair_score + alpha * (probe_prob - 0.5)",
    )
        ...
        for batch in loader:
            ...
            for b in range(len(outputs["candidates"])):
                ...
                for pid in selected_ids:
                    ...
                    aff_pred = int(aff_preds[b, pid].item())
...
def decode_predictions_for_sample(
    ...
    for rp in sample["retained_pairs"]:
        pair_score = rp["pair_score"] if mode == "baseline" else rp["pair_score_calibrated"]
        for cat_id, cat_prob in zip(rp["topcat_ids"], rp["topcat_probs"]):
            decode_score = float(pair_score * cat_prob)
            candidates.append(
                {
                    "asp_span": tuple(rp["a_word"]),
                    "opn_span": tuple(rp["o_word"]),
                    "category_id": int(cat_id),
                    "affective": int(rp["aff_pred"]),
    ...
    for rec in sample_records:
        ...
        if use_calibrated_scores:
            for rp in rec["retained_pairs"]:
                pid = int(rp["pid"])
                if 0 <= pid < len(scores):
                    scores[pid] = float(rp["pair_score_calibrated"])
        out.append(
            {
                "pair_scores": scores,
...
def compute_mode_metrics(
    ...
    breakdown = compute_a_breakdown(sample_records, max_span_length=max_span_length)
...
def main() -> None:
    ...
    if args.output_root:
        ...
        out_root = Path(
            f"outputs/stage1_frozen_probe_replay_{ts}"
        ).resolve()
    ...
    for split_data in [dev_data, test_data]:
        ...
        # Attach probe prob and calibrated score back to retained pairs
        idx = 0
        pred_rows = []
        for rec in split_data["sample_records"]:
            for rp in rec["retained_pairs"]:
                p = float(probs[idx])
                rp["probe_prob"] = p
                rp["pair_score_calibrated"] = float(
                    rp["pair_score"] + args.probe_alpha * (p - 0.5)
                )
                pred_rows.append(
                    {
                        "split": split_data["split"],
                        ...
                        "pair_score_calibrated": rp["pair_score_calibrated"],
                        "label_probe": rp["label_probe"],
                        "label_materializable": rp["label_materializable"],
                        "neg_type": rp["neg_type"],
                        "asp_span_subword": str(rp["a_sub"]),
                ...
                idx += 1
        pred_csv = summary_dir / f"frozen_probe_predictions_{split_data['split']}.csv"
    ...
    (summary_dir / "frozen_probe_model_params.json").write_text(
        json.dumps(
            {
    ...
    metrics_csv = summary_dir / "frozen_probe_replay_metrics_dev_test.csv"
    ...
    (summary_dir / "frozen_probe_replay_deltas.json").write_text(
        json.dumps(deltas, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    (summary_dir / "frozen_probe_replay_invariants.json").write_text(
    ...
    takeaways = [
        "# Frozen Probe Offline Replay (Fixed Aff-ACR checkpoint)",
        "",
        f"- checkpoint: {entry.stage1_ckpt}",
        f"- config: {cfg_path}",
        f"- probe_alpha: {args.probe_alpha}",
        "",
    ...
    (summary_dir / "frozen_probe_replay_takeaways.md").write_text(
        "\n".join(takeaways) + "\n",
        encoding="utf-8",
    )
    ...
    manifest = {
        ...
        "summary_deltas_json": str(summary_dir / "frozen_probe_replay_deltas.json"),
        "summary_invariants_json": str(summary_dir / "frozen_probe_replay_invariants.json"),
    }
    (notes_dir / "frozen_probe_replay_manifest.json").write_text(
