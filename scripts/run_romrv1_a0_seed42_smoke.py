"""Run 1-seed smoke for AGML-BR + A0 + Aff-ACR + ROMR-v1.

Pipeline:
1) Stage-1 train
2) Stage-1 test A-breakdown
3) Stage-1 A1 side split (test)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Seed42 smoke for retained-only materialization auxiliary (ROMR-v1)"
    )
    parser.add_argument(
        "--config",
        default="configs/asqp_rest15_agmlbr_a0_affacr_romrv1.yaml",
        help="ROMR-v1 YAML config",
    )
    parser.add_argument(
        "--output_root",
        default=None,
        help=(
            "Output root (default: "
            "outputs/stage2_e2e_agmlbr_a0_romrv1_seed42_smoke_<timestamp>)"
        ),
    )
...
def main() -> None:
    ...
    if args.output_root:
        ...
        output_root = (
            PROJECT_ROOT / f"outputs/stage2_e2e_agmlbr_a0_romrv1_seed42_smoke_{ts}"
        ).resolve()
    ...
    train_cfg = cfg_dir / "romrv1_a0_train.yaml"
    testeval_cfg = cfg_dir / "romrv1_a0_testeval.yaml"
    ...
    seed = int(args.seed)
    run_name = f"A0_seed{seed}_agmlbr_affacr_romrv1"
    ...
    if args.force or not stage1_ckpt.exists():
        cmd = [
            ...
            "--purpose", "smoke",
            "--run_tag", "agmlbr_affacr_romrv1_smoke",
        ]
    ...
    print("Next: run scripts/summarize_romrv1_a0_seed42_smoke.py")
