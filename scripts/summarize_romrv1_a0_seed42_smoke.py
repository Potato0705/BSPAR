...
baseline AGML-BR+A0 vs strongest Aff-ACR only vs ROMR-v1 variant.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize ROMR-v1 seed42 smoke")
    parser.add_argument(
        "--romr_output_root",
        required=True,
        help="Output root of run_romrv1_a0_seed42_smoke.py",
    )
    parser.add_argument(
        "--baseline_summary",
...
def collect_stage1(stage1_ckpt: str) -> dict:
    ...
    return {
        ...
        "romr_active_aspects": to_float(metrics.get("romr_active_aspects")),
        "romr_active_pairs": to_float(metrics.get("romr_active_pairs")),
        "romr_mean_pos_score": to_float(metrics.get("romr_mean_pos_score")),
        "romr_mean_hardneg_score": to_float(metrics.get("romr_mean_hardneg_score")),
        "romr_violation_rate": to_float(metrics.get("romr_violation_rate")),
        "romr_loss_mean": to_float(metrics.get("romr_loss_mean")),
...
def row_from_romr(romr_root: Path, seed: int) -> dict:
    manifests = list(romr_root.glob("runs/*/run_manifest.json"))
    if not manifests:
        raise RuntimeError(f"No run_manifest.json under: {romr_root / 'runs'}")
    m = load_json(manifests[0])
    if int(m["seed"]) != seed:
        raise RuntimeError(f"ROMR run seed mismatch: expected {seed}, got {m['seed']}")
...
def main() -> None:
    args = parse_args()
    seed = int(args.seed)
    romr_root = Path(args.romr_output_root).resolve()
    summary_dir = romr_root / "summary"
    ...
    romr_row = row_from_romr(romr_root, seed)

    rows = [baseline_row, affacr_row, romr_row]
    fields = [
        "track",
        "seed",
        "dev_quad_f1",
        ...
        "romr_active_pairs",
        "romr_mean_pos_score",
        "romr_mean_hardneg_score",
        "romr_violation_rate",
        "romr_loss_mean",
        "run_dir",
        "stage1_ckpt",
    ]
    compare_csv = summary_dir / "romrv1_smoke_compare_seed42.csv"
    write_csv(compare_csv, rows, fields)

    # delta table: romr vs baseline + romr vs strongest affacr
    delta_rows = []
    for ref in [baseline_row, affacr_row]:
        delta = {"from_track": ref["track"], "to_track": romr_row["track"]}
        for k in fields:
    ...
    delta_csv = summary_dir / "romrv1_smoke_key_deltas_seed42.csv"
    write_csv(delta_csv, delta_rows, delta_fields)

    takeaways = [
        "# ROMR-v1 Smoke (Seed42)",
        "",
        f"- baseline test: {baseline_row['test_quad_f1']:.6f}",
        f"- strongest affacr test: {affacr_row['test_quad_f1']:.6f}",
        f"- romr-v1 test: {romr_row['test_quad_f1']:.6f}",
        f"- delta vs baseline: {romr_row['test_quad_f1'] - baseline_row['test_quad_f1']:+.6f}",
        f"- delta vs strongest affacr: {romr_row['test_quad_f1'] - affacr_row['test_quad_f1']:+.6f}",
    ]
    md = summary_dir / "romrv1_smoke_takeaways_seed42.md"
    md.write_text("\n".join(takeaways) + "\n", encoding="utf-8")
